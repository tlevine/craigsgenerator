    ...
    try:
        ...
        while True:
            ...
            for response in responses:
                yield join(urls[response.url], response, datetime_func, site, section)
            results = []
...
def join(result, response, datetime_func, site, section):
    r = dict(result)
    r.update(parse_listing(response))

    r['html'] = response.text
