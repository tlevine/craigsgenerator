...
def check_bad_scheme(n_threads):
    with n.assert_raises(ValueError):
        r = download(fake_get, fake_warehouse, 'ftp://example.com', None, n_threads = n_threads)

def check_cached(n_threads):
    r = download(fake_get, fake_warehouse, 'http://foo.bar', None, n_threads = n_threads)
    n.assert_equal(r.text, 'baz')

def check_cached_with_date(n_threads):
    r = download(fake_get_should_not_run, fake_warehouse_with_date, 'http://foo.bar', '2014-03-01', n_threads = n_threads)
    n.assert_equal(r.text, 'baz')
...
def check_not_cached(n_threads):
    d = {}
    r = download(fake_get, d, 'http://foo.bar', None, n_threads = n_threads)
    n.assert_equal(r.text, 'lalala')
